...
OMR_Gross_ErrEstG = norm(G - G0,'fro') / (1 + norm(G0,'fro'));
...
CMR_Gross_ErrEstG = norm(G - G0,'fro') / (1 + norm(G0,'fro'));
CMR_Gross_RecRateG = sum( sign(G(:)) == sign(G0(:)) ) / numel(G0);

%% print results
fid = fopen(['../Results/Example1_Results_' num2str(randVar) '.txt'],'w+');
fprintf(fid, ['%8.6f  %8.6f  %8.6f  %8.6f  '...
    ...
    OMR_Gross_ErrPre, OMR_Gross_ErrEstW, OMR_Gross_RecRateW, OMR_Gross_opts.lambda, OMR_Gross_ErrEstG, OMR_Gross_RecRateG,...
    CMR_Gross_ErrPre, CMR_Gross_ErrEstW, CMR_Gross_RecRateW, CMR_Gross_opts.lambda, CMR_Gross_ErrEstG, CMR_Gross_RecRateG);
fclose(fid);