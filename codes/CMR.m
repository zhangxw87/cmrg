function [W, Output] = CMR(X, Y, opts)
%
% ADMM method for the following optimization problem of CMR
%  min \|Y - X * W\|_{2,1} + lambda * R(W)
%
% USAGE:
%  [W, Theta, Output] = CMR(X, Y, opts)
%
% INPUTS:
%
...
%             CMR_opts will be called to obtain the default values.
%
% OUTPUTS:
...
if nargin < 3
    opts = [];
    if nargin < 2;
        error('CMR requires at least two input arguments: X and Y.');
    end
end
...
for Numit = 1:maxitr    
    ...
    if mod(Numit,50) == 0 % compute residuals every 50 iterations
        ...
        % duality gap
        Objvalp = sum(Z_norm) + lambda * sum( Theta_norm );
