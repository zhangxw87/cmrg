function Example5(randVar)
% Example 5
% investigate the effect of grossScale

%% generate data
d = 1000; nTrain = 400; nValid = 400; nTest = 10^4; p = 13;
sigmaMax = sqrt(2);
opts.NoiseType = 'DiffNoise';
opts.gross_rate = 0.2;
ScaleSet = [5 10 50 100];

lambdaSet = (sqrt(log(d)) + sqrt(p)) * 2.^(linspace(-5,5,21));
rhoSet = 2.^(linspace(-5,5,21));

for k = 1:length(ScaleSet)
    opts.grossScale = ScaleSet(k);
    ...
    OMR_ErrPreAdj = norm(bsxfun(@rdivide, Yte - Xte * W, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro') / ...
        norm(bsxfun(@rdivide, Yte, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro');
    ...
    CMR_ErrPreAdj = norm(bsxfun(@rdivide, Yte - Xte*W, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro') / ...
        norm(bsxfun(@rdivide, Yte, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro');
    ...
    OMR_Gross_ErrPreAdj = norm(bsxfun(@rdivide, Yte - Xte*W, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro') / ...
        norm(bsxfun(@rdivide, Yte, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro');
    OMR_Gross_ErrEstW = norm(W - W0,'fro') / norm(W0,'fro');
    OMR_Gross_RecRateW = sum( sign(W(:)) == sign(W0(:)) ) / numel(W0);
    OMR_Gross_ErrEstG = norm(G - G0,'fro') / max(1, norm(G0,'fro'));
    ...
    CMR_Gross_ErrPreAdj = norm(bsxfun(@rdivide, Yte - Xte*W, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro') / ...
        norm(bsxfun(@rdivide, Yte, sigmaMax .* 2.^(-(0:(p-1))/4)),'fro');
    CMR_Gross_ErrEstW = norm(W - W0,'fro') / norm(W0,'fro');
    CMR_Gross_RecRateW = sum( sign(W(:)) == sign(W0(:)) ) / numel(W0);
    CMR_Gross_ErrEstG = norm(G - G0,'fro') / max(1, norm(G0,'fro'));
    CMR_Gross_RecRateG = sum( sign(G(:)) == sign(G0(:)) ) / numel(G0);
    
    %% print results
    fid = fopen(['../Results/Example5_Results_' num2str(randVar) '.txt'],'a+');
    fprintf(fid, ['%8.6f  '...
        ...
        '%8.6f  %8.6f  %8.6f  %8.6f  %8.6f  %8.6f  %8.6f  %8.6f\n'],...
        opts.grossScale, ...
        OMR_ErrPre, OMR_ErrPreAdj, OMR_ErrEstW, OMR_RecRateW, OMR_opts.lambda,...
        CMR_ErrPre, CMR_ErrPreAdj, CMR_ErrEstW, CMR_RecRateW, CMR_opts.lambda,...
        OMR_Gross_ErrPre, OMR_Gross_ErrPreAdj, OMR_Gross_ErrEstW, OMR_Gross_RecRateW, OMR_Gross_opts.lambda, OMR_Gross_opts.rho, OMR_Gross_ErrEstG, OMR_Gross_RecRateG,...
        CMR_Gross_ErrPre, CMR_Gross_ErrPreAdj, CMR_Gross_ErrEstW, CMR_Gross_RecRateW, CMR_Gross_opts.lambda, OMR_Gross_opts.rho, CMR_Gross_ErrEstG, CMR_Gross_RecRateG);
    fclose(fid);
end
