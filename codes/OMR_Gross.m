function [W, G, Output] = OMR_Gross(X, Y, opts)
%
% ADMM method for the following optimization problem of Ordinary Multivariate Regression with gross
% errors
%    (W, G) = argmin_{W,G} 0.5 * \|Y - X * W - G\|_F + lambda * R(W) + rho * \|G\|_1
%
% USAGE:
% [W, G, Output] = OMR_Gross(X, Y, opts)
%
% INPUTS:
%
...
%             OMR_Gross_opts will be called to obtain the default values.
%
% OUTPUTS:
...
if nargin < 3
    opts = [];
    if nargin < 2;
        error('OMR_Gross requires at least two input arguments: X and Y.');
    end
end
...
for Numit = 1:maxitr        
    ...
    % update W
    W = R * (X' * (Y - G) + C' * (sigma * Theta - Lambda));
    ...
    if mod(Numit,50) == 0 % compute residuals every 20 iterations
        ...
        Z = Y - X * W - G;
        ind = (G ~= 0);
        etad2 = sum( (Z(ind) - rho * sign(G(ind))) .^ 2 );
        etad2 = etad2 + sum( max(abs(Z(~ind)) - rho,0) .^ 2 );
        etad2 = sqrt(etad2);
        
        etad3 = norm(X' * Z - C' * Lambda, 'fro') / (1 + norm(C' * Lambda, 'fro'));
        
        etad = max([etad1, etad2, etad3]);
        
        % duality gap
        Objvalp = 0.5 * norm(Z, 'fro')^2 + lambda * sum( Theta_norm ) + rho * sum( abs(G(:)) );
        Objvald = trace(Z' * Y) - 0.5 * norm(Z, 'fro')^2;
