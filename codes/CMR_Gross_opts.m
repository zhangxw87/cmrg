function opts = CMR_Gross_opts(opts)

% Options for CMR_Gross
%
% If opts is empty upon input, opts will be returned containing the default
% options for CMR_Gross.m.  
%
...
%                   ** 1 **
% .rho              regularization parameter for gross error
%                   ** 1 **
...
if isfield(opts,'tol2')
    if (opts.tol2 <= 0) || (opts.tol2 >= 1)
        error('opts.tol2 is tolerance on primal and dual gap. Should be in (0,1).');
    end
else
    opts.tol2 = 1E-4;
end
